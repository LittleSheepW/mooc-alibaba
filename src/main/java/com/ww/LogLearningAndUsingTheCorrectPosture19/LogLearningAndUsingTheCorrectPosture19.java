...
public class LogLearningAndUsingTheCorrectPosture19 {
     ...
     * 换句话说：appender叠加地继承了logger 的层次等级。如果root 日志有一个控制台appender，那么所有启动的日志至少都会输出到控制台中。
     * 如果root下还有一个叫L的logger，它含有一个文件appender,那么L和L的子层次logger的所有日志都将会打印到控制台和文件中。
     * 如何关闭累加行为？
     ...
     *【推荐】面向测试打印日志。所谓的面向测试打印日志是指:方便测试人员测试开发人员代码而提供的测试。此日志的级别一般为DEBUG，一般仅在测试环境可用。有些测试人员并不会拉取你的代码或者编写单元测试，
     * 而只对你的编码进行功能性测试。此时，测试人员构造测试用例对你的接口进行验证时只能观察到接口的返回值，如果你能提供其中的核心计算函数的日志，将极大方便测试人员去验证功能。
     * <p>
     * 6、如果忘记打日志却着急排查问题怎么办?
     * 可以使用Arthas watch检测指定类的指定方法的入参和出参以及异常情况。
     *
     * <p>
     * 1、学习设计模式我们主要思考以下几个问题:
     * (1)为什么会出现这种设计模式?
     * 门面模式：为了隐藏系统复杂性，向使用方提供统一的可以访问系统的接口。
     * <p>
     * (2)这种设计模式的核心思想是什么?
